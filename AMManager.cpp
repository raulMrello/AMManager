				...
				for(auto r = dobj->readings->begin(); r != dobj->readings->end(); ++r){
					...
					if(keys != 0){
						if(keys & AMDriver::ElecKey_Voltage){
							...
							DEBUG_TRACE_D(_EXPR_, _MODULE_, "Analizador=[%d], Voltage=%d", (base_analyzer + amr->analyzer),(int)_amdata.analyzers[(base_analyzer + amr->analyzer)].stat.measureValues.voltage);
						}
						if(keys & AMDriver::ElecKey_Current){
							...
							DEBUG_TRACE_D(_EXPR_, _MODULE_, "Analizador=[%d], Current=%d", (base_analyzer + amr->analyzer),(int)_amdata.analyzers[(base_analyzer + amr->analyzer)].stat.measureValues.current);
						}
